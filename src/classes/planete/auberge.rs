...
impl Auberge {
    ...
    pub fn proposer_repos(&self, personnage_rc: Rc<RefCell<PersonnagePrincipal>>, choix: Option<u8>) {
        let mut personnage = personnage_rc.borrow_mut();
        AffichageDeplacement::lancer_animation("auberge", self.phrase_arrive.clone());
